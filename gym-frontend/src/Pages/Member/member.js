...
const Member = () => {
    ...
    const fetchData = async (skip, limits) => {
        ...
        await axios.get(`http://localhost:4000/members/all-member?skip=${skip}&limit=${limits}`, { withCredentials: true }).then((response) => {
            ...
            toast.error("Login is not Valid anymore")
            console.log(err)
            localStorage.clear()
            navigate('/')
        })
    ...
    return (
        <div className='text-black p-2 w-3/4 h-[100vh]'>
            ...
            <Link to={'/dashboard'} className='border-2 pl-3 pr-3 pt-1.5 pb-2 rounded-2xl cursor-pointer text-white bg-slate-900 hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:text-black'>
            ...
            <div className='bg-slate-100 p-5 mt-5 rounded-lg grid gap-2 grid-cols-3 overflow-x-auto h-[55%]'>
                {
                    data.map((item, index) => {
                        return (
            ...
            {addMember && <Modal header={"Add New Member"} handleClose={handleMembers} content={<Addmembers handleClose={handleMembers} />} />}
            <ToastContainer />
        </div>
    )
}
